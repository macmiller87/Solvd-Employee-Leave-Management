...
describe("Delete Vacation (Unit Test)", () => {
    ...
    it("should be able to delete an employee's vacation", async () => {
        ...
        const del = await inMemoryVacationRepository.deleteVacation({
            vacation_id: calcVacation.vacation[0].vacation_id,
            token: token
        })

        expect(del).toStrictEqual({ message: "Vacation deleted with success !" });
    });
    ...
    it("shouldn't be able to delete an employee's vacation, if 'vacation_id' parameter aren't correct", async () => {
        ...
        await expect(
            inMemoryVacationRepository.deleteVacation({
                vacation_id: fakeId,
                token: token
